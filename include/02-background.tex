...
interacting walls are computed. These are passed to the structural solver as boundary conditions. The structural solver computes the deformations, with which then the fluid mesh is modified. Afterwards the flow solver is started again. 

The FSI sub-iteration loop is repeated until a convergence criteria $\epsilon$ is satisfied, which is determined by the change in displacements,
